...
import { radius, spacing, typography, useTheme, colors as mapColors, type Palette } from '../theme';
...
function SitesPanel({
  ...
  const renderGridItem = ({ item }: { item: TouristSite }) => (
    ...
    >
      <RemoteImage uri={item.image} style={styles.gridImage} fallbackLabel={item.name} />
      {__DEV__ ? <Text style={styles.debugUrl} numberOfLines={1}>{item.image}</Text> : null}
      {item.isPremium ? (
  ...
  const renderListItem = ({ item }: { item: TouristSite }) => (
    <Pressable
      ...
      onPress={() => navigation.navigate('SiteDetail', { site: item })}
    >
      <RemoteImage uri={item.image} style={styles.listImage} fallbackLabel={item.name} />
      {__DEV__ ? <Text style={styles.debugUrl} numberOfLines={1}>{item.image}</Text> : null}
...
const createStyles = (colors: Palette) => StyleSheet.create({
  ...
  debugUrl: { marginTop: 6, fontSize: 10, color: colors.textMuted },
  ...
  mapPanel: { flex: 1 },
  mapWrap: { flex: 1, marginHorizontal: spacing.lg, borderRadius: radius.lg, overflow: 'hidden', minHeight: 360 },
